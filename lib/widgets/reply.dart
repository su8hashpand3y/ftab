...
import 'package:flutter_tab/widgets/replyMessageWidget.dart';
...
class ReplyWidgetState extends State<ReplyWidget> {
  ...
  Future _loadData() async {
     final res =  await Internet.get('http://127.0.0.1:58296/Message/GetReplyMessageCard');
     if(res.status == 'bad'){
     ...
     setState(() {
       _data  = new List<MessageCard>();
          for (var item in res.data) {
              _data.add(new MessageCard(item["userName"],item["messageGroupUniqueGuid"],item["unreadCount"],item["lastMessage"],item["isFav"]));
           }
          });
     }
    }
    ...
    await Navigator.of(context).push(
        new MaterialPageRoute<dynamic>(
          builder: (BuildContext context) {
            return new ReplyMessageWidget(message);
          },
        ));
  }
          ...
          Flexible(
            ...
            child: ListView.builder(
              ...
              itemBuilder: (context, int index) {
                return  RaisedButton(
                     onPressed: (){ this._openMessage(_data[index]); },
